function [Q_l_NNN,coord_distance]= obtain_Q_l_2ndNN_inds_PBC(Model,BondTh,l,CellPara, PBCEnforceYN,coord)
...
coord_distance = zeros(size(Model,2),coord);
for i=1:size(Model,2)
    ...
    [sortedDistance , sortedInds] = sort(Dist);

    BondAtomInds = sortedInds(2:coord+1);
    coord_distance(i,:) = sortedDistance(2:coord+1);
